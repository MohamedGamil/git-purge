...
use crate::model::{
    Activity, Classification, MergeState, MetricDelta, NamingVerdict, Repository, ScanResult,
    TrendComparison, TrendHistory,
};
...
pub fn generate_audit_report(
    ...
    md.push_str(&format!(
        "| **Total Branches** | {} | 100.0% | All tracked branches in the mirror |\n",
        total
    ));
    md.push_str(&format!(
        "| **Active Branches** | {} | {} | Branches with commits in the active window |\n",
        active,
        pct(active)
    ));
    md.push_str(&format!(
        "| **Stale Branches** | {} | {} | Branches with last commit older than threshold |\n",
        stale,
        pct(stale)
    ));
    md.push_str(&format!(
        "| **Merged Branches** | {} | {} | Branches fully merged into the default branch |\n",
        merged,
        pct(merged)
    ));
    md.push_str(&format!(
        "| **Unmerged Branches** | {} | {} | Branches with unmerged work |\n",
        unmerged,
        pct(unmerged)
    ));
    md.push_str(&format!(
        "| **Non-Standard Naming** | {} | {} | Branches violating the naming policy |\n\n",
        non_standard,
        pct(non_standard)
    ));
    ...
    if violations.is_empty() {
        ...
        for v in &violations {
            let issue = match &v.naming {
                NamingVerdict::NonStandard { reason } => {
                    match reason {
                        crate::model::NamingViolation::NoCategoryPrefix => {
                            "Missing category prefix (should contain a '/' e.g. feature/name)"
                                .to_string()
                        }
        ...
        let render_stale_group = |group: &[&Classification], title: &str, md_str: &mut String| {
            ...
            if group.is_empty() {
                ...
                for s in group {
                    ...
                    let merged_str = if s.merge_state == MergeState::Merged {
                        "✅ Yes"
                    } else {
                        "❌ No"
                    };
    ...
    for c in &scan.classifications {
        ...
        if name.starts_with("feature/") || name.starts_with("feat/") {
            features.push(c);
        } else if name.starts_with("bugfix/")
...
pub fn generate_trend_report(
    ...
    md.push_str(&format!(
        "# Git Purge Cleanup Progress & Trend Report — {}\n\n",
        repo.display_name
    ));
    md.push_str(&format!(
        "Generated at: {}\n\n",
    ...
    if history.entries.is_empty() {
        md.push_str(
            "No recorded history entries found. Run scans or cleanups to seed the timeseries.\n",
        );
        return md;
    }
