...
use crate::model::{MergeState, Repository, Snapshot, SnapshotId, SnapshotRef, SnapshotTrigger};
...
pub fn create_snapshot(
    ...
    let mut tx = mirror_repo.transaction().map_err(|e| {
        crate::GitPurgeError::Git(format!("Failed to start ref transaction in mirror: {}", e))
    })?;
    ...
    for branch in branches {
        ...
        let class = classifications
            .iter()
            .find(|c| c.branch == branch.name && c.scope == branch.scope);
        ...
        tx.set_target(&backup_ref_path, tip_oid, None, "Git Purge snapshot create")
            .map_err(|e| {
                crate::GitPurgeError::Git(format!(
                    "Failed to set target for backup ref {}: {}",
                    backup_ref_path, e
                ))
            })?;

        // Walk and count reachable commits
        let mut revwalk = mirror_repo
            .revwalk()
            .map_err(|e| crate::GitPurgeError::Git(format!("Failed to get revwalk: {}", e)))?;
        revwalk.push(tip_oid).map_err(|e| {
            crate::GitPurgeError::Git(format!("Failed to push tip OID to revwalk: {}", e))
        })?;
    ...
    tx.commit().map_err(|e| {
        crate::GitPurgeError::Git(format!("Failed to commit ref transaction in mirror: {}", e))
    })?;

    // 4. Construct the metadata manifest JSON
    let manifest_path = mirror_manager
        .resolve_mirror_path(&repo.id)
    ...
    let json_bytes = serde_json::to_vec_pretty(&snapshot).map_err(|e| {
        crate::GitPurgeError::Config(format!("Failed to serialize snapshot manifest: {}", e))
    })?;

    // Save manifest as a git blob inside the mirror repo
    let blob_oid = mirror_repo.blob(&json_bytes).map_err(|e| {
        crate::GitPurgeError::Git(format!("Failed to write manifest blob to mirror: {}", e))
    })?;
    ...
    mirror_repo
        .reference(
            ...
            blob_oid,
            true,
            "Git Purge snapshot metadata",
        )
        .map_err(|e| {
            crate::GitPurgeError::Git(format!("Failed to create metadata reference: {}", e))
        })?;
