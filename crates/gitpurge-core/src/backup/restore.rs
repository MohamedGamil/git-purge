...
use crate::model::{Repository, Snapshot, SnapshotId, RestoreSpec, RestoreOutcome};
...
pub fn restore_snapshot(
    ...
    let meta_ref = mirror_repo.find_reference(&meta_ref_path)
        .map_err(|e| crate::GitPurgeError::Snapshot(format!("Snapshot meta reference not found: {}", e)))?;
    ...
    let ref_entry = snapshot.refs.iter()
    ...
    let target_name = spec.target_name.clone().unwrap_or_else(|| ref_entry.branch.0.clone());
    ...
    let mut remote = source_repo.remote_anonymous(&mirror_path.to_string_lossy())
        .map_err(|e| crate::GitPurgeError::Git(format!("Failed to create anonymous remote for source repo: {}", e)))?;

    let force_prefix = if spec.force { "+" } else { "" };
    let refspec = format!(
        "{}{}:{}",
        force_prefix,
        ref_entry.backup_ref,
        target_ref_path
    );

    let mut fetch_opts = git2::FetchOptions::new();
    remote.fetch(&[&refspec], Some(&mut fetch_opts), None)
        .map_err(|e| crate::GitPurgeError::Git(format!("Failed to fetch restored ref from mirror: {}", e)))?;
